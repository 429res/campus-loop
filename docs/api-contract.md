> 2026-09-09 整合说明：分类写操作、本人物品编辑/下架、正式交换、通用举报和履历核验页面已接入下述现有契约。管理员 `/api/admin/exchanges` 为只读总览，不代替参与者权限。争议裁决仍未实现；本次没有新增或改写 Flyway 迁移。
...
- 详情页直接选择本人的可用物品发起 direct-v1 自主交换（见下节），保留精确版本、幂等、审核和占用检查。


## 个人主页与自主交换增量（2026-09-09）

新增公开个人主页与头像展示，公开物品列表排除已交换物品；个人资料编辑进入独立页面。校园动态关联物品返回实时状态，交换订单列表按物品标题和图片展示，详情独立切换。

契约变更：新增 GET /api/members/{id} 与 /items，仅公开昵称、头像、简介、校区及公开计数，不返回联系方式/账号权限；ItemView 添加 ownerAvatarUrl；Post 添加 itemStatus；ExchangeView.Flow 添加物品标题/图片，新增交换保存图片快照，旧订单兼容读取。

规则变更：推荐继续按分类和标签解释排序；用户自主选择采用显式 direct-v1 双人交换，需求不对口时二次确认。服务端不放宽身份、审核、可用状态、唯一占用、版本及幂等校验；自主交换不占用或履行未匹配的独立需求，双方仍需确认及交接。新增领域和隔离 MySQL 完整生命周期/并发检查。

消息使用前台 5 秒轮询展示站内浮层与红点；隐藏页停止，切换账号清空并隔离迟到响应。原始 60 分改为可理解的分类/标签匹配依据。
