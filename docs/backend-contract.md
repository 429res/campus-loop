...
每件物品当前绑定一个想要分类与一组偏好标签，作为“我有什么 / 想要什么”最小样例。独立需求清单、多条候选需求和收藏尚未实现；本人显示名称编辑与修改密码已按上文契约开放。
...
受保护接口使用 `Authorization: Bearer <token>`。JWT 8 小时到期，MySQL 会话记录同时校验；注销删除当前会话。禁用账号即时拒绝现有会话。本人修改密码已实现，并在同一事务删除此账号所有会话；管理员重置密码仍未实现，后续也必须遵守相同撤销规则。
