...
每件物品当前绑定一个想要分类与一组偏好标签，作为“我有什么 / 想要什么”最小样例。B-01 分支新增独立需求 CRUD 与本人物品候选关联，接口和兼容边界见 [api-contract.md](api-contract.md) 与 [b01-independent-demands.md](b01-independent-demands.md)。独立需求接入匹配和收藏尚未实现；本人资料编辑已由 A-01 提供。
...
HTTP GET `/api/matches` 只读即时计算，不持久化推荐，不创建交换，不写 `cl_item_hold`。推荐 ID 是当次规则生成的环标识，不能作为预约凭证。当前最多支持 200 件 AVAILABLE 候选；超出返回 422 并明确需要候选分区，避免隐藏截断或数据库长时间阻塞。下一阶段可按学校/分类建图、预计算邻接边和分页结果。
...
受保护接口使用 `Authorization: Bearer <token>`。JWT 8 小时到期，MySQL 会话记录同时校验；注销删除当前会话。禁用账号即时拒绝现有会话。修改密码、管理员重置密码后须在同一事务删除此账号所有会话，此类接口尚未实现。

当前运行基础面向四人本地开发。公开部署前补充登录限速、注册/找回密码、学校身份策略、细粒度权限、反垃圾、上传生命周期清理、审计和运维指标；这些是后续范围，不是假装已完成的入口。数据库与 JWT 密钥仅由后端加载，前端不能持有这些凭据。
