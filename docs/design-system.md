...
图标使用固定正方形，不参与文字压缩；按钮通过 `gap` 为图标与文字留空。插画不覆盖表单或主要行动，文字、图标底板、外壳不重复叠加玻璃。窄屏长表格在内部滚动，固定操作列不得遮挡内容。H5 `uni-form` 的内部包装层需要透传布局间距。

用户端共用 `LoopIcon` 与仓库内图集，主导航显式传入 `active-tab`；新 Tab 页面沿用同一机制。图标源文件、导出规范、实际验证和平台边界见[前端修复记录](frontend-refinement.md)。
