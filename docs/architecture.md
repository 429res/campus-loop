...
注册采用默认关闭的准入模式。生产可启用 `EMAIL_VERIFIED`：先发送邮箱验证码，再在注册时消费有效、未使用的验证码，建立邮箱唯一约束；历史账号通过 V20 获得 legacy_exchange_access，新注册不继承该豁免。开发隔离环境可启用 `DEVELOPMENT_SELF_SERVICE`，服务端才在事务中将经过校验的公共字段写成唯一用户名、BCrypt 哈希及固定 `USER/ACTIVE`；客户端不能提供角色、状态、密码哈希或其他用户字段。数据库用户名唯一约束是并发最终防线，冲突整笔回滚为409。成功注册不创建会话，仍通过现有登录流程验证账号状态并建立数据库会话。此开发模式不证明校园身份；校园身份提供方和邀请码仍未接入。账户安全邮件提醒由持久 outbox 异步投递；千问只有读取当前授权上下文的工具，不参与交换状态机或自动审核决定。
...
V17 起，发布物品的 wanted 字段与一条带 source_item_id 的需求在同一事务中同步；V17 为尚未交换完成的既有物品补齐关联需求。物品发布、内容编辑、重新上架维护自动需求，用户通过物品入口编辑它；手动需求继续独立维护。匹配仍只读取 cl_demand/cl_demand_item，并检查本人归属、审核通过、未占用和 ACTIVE 状态，不自动建立交换。
...
B-01实际模型已随PR #4合入，V4是需求结构来源。B-02不新增迁移，保留旧公开 `/api/matches` 的物品wanted字段来源（legacy-v1），新增登录后的 `/api/matches/independent` 读取需求表并只返回含当前用户的环；V17 将发布意向同步到需求表后，两种需求都可参与相同规则。没有结果时不回退旧算法。手动需求停用/删除不会停用物品的自动需求；用户可通过下架物品停止该物品参与匹配。
...
认证与上传的应用层限流当前是单 JVM 内的有界固定窗口：匿名登录/注册按可信来源地址，已认证改密/上传按数据库用户 ID；原子 `ConcurrentHashMap.compute` 保证同一进程并发额度不超发。默认不信任转发头，只在直连地址属于显式代理 IP/CIDR 时从转发链右侧剥离可信跳。429 携带相对等待秒数和绝对 UTC 恢复时刻，窗口到期自然恢复。该状态不持久化，重启会重置；多实例需要统一网关或共享原子存储，当前单实例证据不代表集群一致性。完整边界见[A-05限流说明](a05-auth-upload-rate-limit.md)。
...
自动需求在 owner → demand → item 的顺序中维护，物品变更与需求版本推进原子提交。审核在物品锁前按 ID 锁定操作人和物品拥有者，避免审核通知的用户外键与发布编辑相互等待。数据库悲观锁冲突由事务回滚后映射为 409，不作为成功写入返回。

动态、回复按时间与 ID 分页；纯文本与本人公开上传引用，物品必须属于发布者且已公开。作者撤回与管理员隐藏/恢复区分，举报保留处理状态及审计。发布与回复按作者及请求键幂等，点赞按 post/user 唯一；回复与通知同事务保存，交叉回复按用户 ID 排序锁定后再锁帖子。COMMUNITY 是独立管理权限。账户维护按 USERS 查询和管理普通资料，管理员写入/权限与密码重置由 ALL 控制；密码重置撤销原会话，密码不进入审计。


## 自主双人交换与公开主页（2026-09-09）

按同学试用反馈增加显式 `direct-v1`：需求分类与偏好作为选择参考，由用户二次确认后主动邀请。只允许两位不同用户各提供一件审核通过且未占用的物品；版本、唯一占用、身份与事务锁仍由原创建服务校验。推荐继续采用 `independent-v2` 的分类硬匹配与标签排序。

两种创建模式共用 cl_exchange、参与人、物品占用、确认/取消/超时/争议/交接状态机。direct-v1 不写 cl_exchange_demand，也不履行无关的独立需求；订单快照补充物品标题与图片，避免以后资料变化改写交换时的内容。没有新增数据库迁移。

公开个人主页只投影昵称、头像、校区、简介及统计，联系方式仅本人可见；公共商品查询排除 EXCHANGED，历史详情与动态引用保留当前状态。消息在前台以 5 秒间隔轮询，按账号持久去重；路由隐藏/浏览器后台暂停，退出登录立即清空，迟到响应不能跨账号渲染。
