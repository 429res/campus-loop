...
暂定 V3 新增结构，A 的序号协调与 A/D 的候选基数复核记录在 B-01 Issue/PR；尚未收到确认。B-02 再确认新旧需求的切换、每条流向选择哪条需求、requiredTags/最低成色是否为硬条件及规则版本，B-01 不开放这些未定字段。详见 [B-01 接入说明](b01-independent-demands.md)。
