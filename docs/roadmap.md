...
- A-02 本人物品切片已提供共用 `ItemView.version`，审核写接口、审核元数据及状态筛选仍未提供；D-01 仍按直发 `AVAILABLE` 工作，B-02 仍只消费 `AVAILABLE`。因此真实审核、并发冲突和跨端状态迁移未完成，不能将 C-01 标记完成。
...
公开注册或真实校园身份准入、超出 `displayName` 的个人资料编辑、内容审核写操作、交换正式创建与占用/确认/交接、自动超时、举报裁决、履历审核、外部通知、OSS、真实微信身份认证。相关页面只能显示“待开发”；后端预留操作返回明确501，不允许给出虚构业务成功。
