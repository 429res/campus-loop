...
本功能分支实现按会话的本人分页/详情、完整编辑与 AVAILABLE → HIDDEN 下架、白名单/上传归属校验、版本冲突和占用/进行中交换保护；复用既有表结构，无新增迁移。接入与验证见 [a02-own-items.md](a02-own-items.md)，协作见 [Issue #15](https://github.com/429res/campus-loop/issues/15)。D 的表单和错误契约已同步，尚未完成 D 页面接入确认、H5/微信联调。重新上架、HIDDEN 编辑、审核、收藏、分类写入和正式交换不属于本批，整个 A-02 仍未完成。

争议证据上传与管理员裁决、维修/流转事件、通知 outbox、统计口径、匹配候选索引/分页/缓存、风控及校园身份核验。外部 OSS 和微信登录由适配器接入，必须配置和验证后才开放入口。

## 当前明确未实现

公开注册或真实校园身份准入、超出 `displayName` 的个人资料编辑、收藏持久化、内容审核写操作、交换正式创建与占用/确认/交接、自动超时、举报裁决、履历审核、外部通知、OSS、真实微信身份认证。相关页面只能显示“待开发”；后端预留操作返回明确501，不允许给出虚构业务成功。
