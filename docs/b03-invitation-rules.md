...
同步已合入B-03.1后，`node scripts/run.mjs test`通过后端159项（69公共+90API）及用户端49项；`CAMPUS_TEST_PORT=3332 node scripts/mysql-test.mjs`在一次性MySQL 8.4.11通过同组159项并执行V1–V7迁移，容器自动停止删除。本片新增17纯规则+2集成边界测试，未新增迁移。构建/云端检查以PR实际结果为准，详见[验证记录](verification.md)。数据库测试只能证明实际时钟输入、既有迁移/回归和当前无写入边界，不能证明正式创建或确认/取消事务竞态、唯一释放、回滚、定时扫描恢复。A-03到位后必须用真实创建再走2/3人READY、竞争取消及占用归属验收，并复用A-04共用事务测试。
