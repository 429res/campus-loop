...
## 图片旋转与大幅背景上线（2026-09-10）

应用提交 `13951f1`，PR [#62](https://github.com/429res/campus-loop/pull/62)，范围见[图片与背景调整](image-cover-experience.md)。

- 发布包 SHA-256 `3c44eb9d8470afd563ce34a58678ba2ddd8f7c4ac224431ca995a1b2cab95f46`，发布前备份 `.local/backups/20260910T080150Z`。后端/Web 镜像均切至 `13951f1`，服务健康，备份定时器 active；数据库保持 V22，无新增迁移。
- 线上实测 20 MP PNG 上传成功、自动优化后旋转宽高正确交换并生成新文件；验收图不关联真实个人资料或已发布内容。临时验收登录会话已退出。
- 公网两域健康、既有业务读取、公共头像与前端入口脚本/图标哈希检查通过。我的、资料编辑和公开主页的背景显示、旋转后保存同步在本机浏览器验证；390 px 宽度无横向溢出。
- 按用户要求未运行复杂数据库回归；后端打包与 H5、小程序构建通过。此变更未新增外部模型调用或服务依赖。
