...
2026-09-08：H2与全新一次性MySQL 8.4.11均通过8项匹配、18项集成测试（共26项，新增9项B-01）。另一个临时MySQL容器先应用V1/V2和原有测试夹具，再升级V3并通过26项测试，逐行核对旧物品全部字段未改变。两个容器均已自动停止并删除，未连接成员日常库。API通过MockMvc进入真实认证/服务/Mapper，持久化与并发版本冲突实际发生在MySQL；不等同于浏览器或微信联调。

接口、边界及逐层证据见 [B-01 接入说明](b01-independent-demands.md)。B-02匹配与正式交换未接入；A的迁移序号及A/D候选基数确认、其他成员审核和PR CI仍须合并前完成。
