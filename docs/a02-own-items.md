...
2026-09-08 同步 B-02 主线后的最终运行：`node scripts/run.mjs test` 的 UniApp Node 测试26项、后端87项（42匹配 + 45 H2 API集成）全部通过。`node scripts/mysql-test.mjs` 在全新临时 MySQL 8.4.11 容器从空库运行 V1–V4，同一87项后端测试全部通过，其中45项API集成实际连接MySQL，0失败/错误/跳过；容器自动停止移除，未连接日常开发库。

新增8组物品集成测试在两种数据库均通过：数据库回读、六状态本人隔离、完整表单白名单/类型/边界、真实上传归属与移除引用、版本冲突、三种编辑/下架竞态、8个等待物品锁后的归属/占用/状态/交接引用复核场景、下架退出公开读取以及旧/独立需求两种推荐、履历和独立需求关联保留。交换写入仅为隔离测试夹具，没有正式交换API联调。
