...
D：PR #18需求页已在本批提交期间合入main，本批已同步并复核。发布和需求选择继续 GET /api/categories 默认 ACTIVE，按返回顺序渲染；不要传 includeInactive=true 到新业务选择器。失效分类的草稿保留正文并要求重选，分类全停用返回[]也要清除旧ID，本批修正了发布页这个空数组边界。首页用于浏览历史物品的分类筛选改为 includeInactive=true。已打开的旧表单可能收到400，保留输入并刷新分类；没有刷新也由服务端拒绝停用目标。需求编辑的原分类不在选项中时应显示未选择，不能静默改用第一个分类；启用需求失败可先编辑改选或保持停用。需求页刷新分类时按所选ID重新定位下标，避免分类调整排序后静默选到另一项；最新请求优先，失效项清空、无选项禁用，保存400后刷新分类且保留输入/version。新增5项真实页面脚本回归覆盖这些行为。
...
- 根 `node scripts/run.mjs test`：用户端32项；后端109项（公共模块42、API67），失败/错误/跳过均0，其中新增分类集成测试12项。
...
- 同步PR #18后重跑根test（用户端32、后端109），H5/微信构建及微信产物6项通过；浏览器对合入的需求页验证分类停用400后显示真实错误、保留说明和version=3、刷新为空选项并禁用选择器，使用HTTP夹具，不写真实数据库。
- `node scripts/repository-check.mjs`、`git diff --check` 通过。

数据库测试通过MockMvc调用实际控制器、事务服务和真实MySQL，并执行直接约束SQL；未启动独立后端HTTP进程做C/D页面与MySQL联调。没有微信开发者工具/真机或本批减少动态效果实机检查，没有完成C正式维护表单或C/D页面连接真实MySQL的联调。后续由消费方审核和联调，合并仍需另一成员审核与CI成功，不自动合并。
