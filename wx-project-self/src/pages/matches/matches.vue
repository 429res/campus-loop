    ...
    <template v-else>
      ...
      <view v-else class="match-list">
        <view v-for="match in visible" :key="match.id" class="cl-panel match-card">
          <view class="match-card-header"><view class="cl-row"><text class="cl-tag" :class="{'cl-tag--pink':match.length===3}">{{ match.length === 2 ? '双方交换' : '三方循环' }}</text><text class="match-card-title">{{ match.length }} 位同学，各得所需</text></view><view class="score"><text>{{ match.score }}</text><text class="cl-hint">匹配分</text></view></view>
