...
const section = ref(0), readiness = ref(null), viewerId=ref(null)
const received=match=>match.flows.find(flow=>flow.toUserId===viewerId.value)?.itemTitle
const offered=match=>match.flows.find(flow=>flow.fromUserId===viewerId.value)?.itemTitle
const publish = () => uni.switchTab({url:'/pages/publish/publish'})
const myItems = () => uni.navigateTo({url:'/pages/my-items/my-items'})
const readinessTitle = computed(() => recommendations.value.length ? `为你找到 ${recommendations.value.length} 个交换方案` : !readiness.value ? '发布闲置，寻找合适的交换' : readiness.value.AVAILABLE ? '正在为你的闲置寻找交换' : readiness.value.PENDING_REVIEW ? '物品正在审核' : readiness.value.RESERVED ? '你有正在进行的交换' : '先发布一件闲置吧')
const readinessText = computed(() => recommendations.value.length ? '看看彼此的物品，选一个合适的方案发出邀请吧。' : !readiness.value ? '填写你有什么、想换什么，审核通过后就会自动匹配。' : readiness.value.AVAILABLE ? `你有 ${readiness.value.AVAILABLE} 件可交换物品。遇到彼此合适的需求，方案就会出现在这里。` : readiness.value.PENDING_REVIEW ? `${readiness.value.PENDING_REVIEW} 件物品等待审核，通过后自动参与匹配，无需再次填写需求。` : readiness.value.RESERVED ? '等待大家确认并完成交接，可在“我的交换”查看进度。' : '告诉大家你有什么、想换什么。审核通过后，我们会自动寻找交换方案。')
...
async function load() {
  alive = true
  restoreCreation()
  viewerId.value=Number(uni.getStorageSync(USER_KEY)?.id)||null
  preview.value = null
  ...
  try {
    ...
    const [data, counts] = await Promise.all([activeRequest, http.get('/api/matches/readiness',{}, {silent:true})])
    if (!requestGuard.isCurrent(ticket)) return
    readiness.value = counts
...
<template>
  <LoopLayout active-tab="matches">
    <view class="match-heading"><view class="cl-page-heading"><text class="match-kicker">A LITTLE MATCH, A NEW LOOP</text><text class="cl-title">交换灵感</text><text class="cl-subtitle">看看有哪些合适的交换，也和同学聊聊闲置的新去处。</text></view><view class="match-art" aria-hidden="true"><text class="art-bubble">你有的，正好是我需要的</text><view class="art-exchange"><image src="/static/demo/book.svg" mode="aspectFit"/><LoopIcon name="exchange" tone="primary" :size="36"/><image src="/static/demo/lamp.svg" mode="aspectFit"/></view></view></view>
    <view class="section-tabs" role="group" aria-label="交换与动态"><LoopButton v-for="(label,index) in ['为你匹配','校园动态']" :key="label" class="section-tab" :class="{selected:section===index}" :aria-pressed="section===index" @click="section=index"><LoopIcon :name="index?'user':'exchange'" :tone="section===index?'primary':'default'" :size="28"/><view><text>{{label}}</text><text class="cl-hint">{{index?'看看同学在分享什么':'选择物品，发起交换'}}</text></view></LoopButton></view>
    <CommunityFeed v-if="section===1"/>
    <template v-else>
    <view class="cl-notice recommendation-help">推荐根据双方需求分类与偏好标签排序，不代表物品价值或成交概率。看到喜欢的物品，也可以进入详情自主选择交换。</view><view class="match-toolbar"><LoopSegment v-model="filter" :options="['全部推荐','双方交换','三方循环']"/><LoopButton class="cl-btn" :disabled="loading || !authenticated" @click="load">↺ 更新推荐</LoopButton></view>
    ...
    <view v-if="authenticated" class="cl-panel match-notice"><view><text class="cl-section-title">{{ readinessTitle }}</text><text class="cl-hint">{{ readinessText }}</text></view><view class="notice-actions"><LoopButton class="cl-btn cl-btn--primary" @click="publish">发布闲置</LoopButton><LoopButton class="cl-btn" @click="myItems">我的物品</LoopButton><LoopButton class="cl-btn" @click="openExchanges">我的交换</LoopButton><LoopButton class="cl-btn" @click="openDemands">求换清单</LoopButton></view></view>

    <view v-if="!authenticated" class="cl-panel cl-empty"><LoopIcon name="user" tone="primary" :size="36"/><text>{{ authNotice || '登录后查看适合你的交换' }}</text><text class="cl-hint">发布闲置后，合适的交换方案会自动出现在这里。</text><LoopButton class="cl-btn cl-btn--primary" @click="login">{{ authNotice ? '重新登录' : '登录' }}</LoopButton></view>
    <LoopSkeleton v-else-if="loading" />
    <view v-else-if="error" class="cl-panel cl-empty" role="alert"><text class="cl-error">{{ error }}</text><text v-if="errorStatus===422" class="cl-hint">当前参与匹配的物品较多，请稍后再试。</text><LoopButton class="cl-btn" @click="load">重新读取</LoopButton></view>
    <view v-else-if="!recommendations.length" class="cl-panel cl-empty"><LoopIcon name="exchange" tone="primary" :size="36"/><text>还没有找到合适的交换</text><text class="cl-hint">匹配需要双方或三方都能换到想要的物品。你可以去校园动态说说需求，或稍后回来看看。</text></view>
    <template v-else>
      <view v-if="!visible.length" class="cl-panel cl-empty"><text>当前筛选下没有推荐</text><text class="cl-hint">试试切换到“全部推荐”。</text></view>
      <view v-else class="match-list">
        <view v-for="match in visible" :key="match.id" class="cl-panel match-card">
          <view class="match-card-header"><view class="cl-row"><text class="cl-tag" :class="{'cl-tag--pink':match.length===3}">{{ match.length === 2 ? '双方交换' : '三方循环' }}</text><text class="match-card-title">{{ match.length }} 位同学，各得所需</text></view><view class="match-basis"><text class="cl-tag">分类符合</text><text class="cl-hint">{{match.flows.some(f=>f.matchedTags?.length)?'命中偏好标签 · 优先推荐':'暂无共同标签，按分类推荐'}}</text></view></view>
          <view class="your-exchange"><text>你拿出：{{offered(match)}}</text><text>你换到：{{received(match)}}</text></view>
          <view class="participant-list"><view v-for="participant in match.participants" :key="participant.userId" class="participant"><MemberLink :id="participant.userId" :name="participant.displayName" :show-name="false"/><view><text class="cl-field-title">{{ participant.displayName }}</text><text class="cl-hint">提供：{{ participant.itemTitle }}</text></view></view></view>
          ...
          <view class="flow-list">
            <view v-for="(flow,index) in match.flows" :key="`${flow.itemId}-${flow.toUserId}`" class="flow-row">
              ...
              <view class="flow-main"><view class="flow-people"><text>{{ flow.fromName }}</text><text class="flow-arrow">→</text><text>{{ flow.toName }}</text></view><LoopButton class="flow-item" @click="openItem(flow.itemId)">{{ flow.itemTitle }} ↗</LoopButton><view class="demand-proof"><text class="hard-rule">想要 {{ flow.matchedCategoryName }}</text><text v-if="flow.matchedTags?.length" class="soft-rule">共同偏好：{{ flow.matchedTags.join('、') }}</text></view></view>
            </view>
          </view>

          <view class="match-pending"><text class="cl-hint">先看看物品与交换顺序，合适的话就邀请大家。</text><LoopButton class="cl-btn cl-btn--primary" @click="previewMatch(match)">查看方案</LoopButton></view>
        </view>
      </view>
    </template>
    <LoopSheet :model-value="!!preview" title="发起交换前确认" @update:model-value="value => { if(!value) closePreview() }">
      <view v-if="preview" class="proposal-preview">
        <view><text class="cl-section-title">{{ preview.length }} 人交换方案</text></view>
        <view class="preview-participants"><view v-for="participant in preview.participants" :key="participant.userId" class="preview-person"><MemberLink :id="participant.userId" :name="participant.displayName" :show-name="false"/><view><text class="cl-field-title">{{ participant.displayName }}</text><text class="cl-hint">提供：{{ participant.itemTitle }}</text></view></view></view>
        <view class="cl-divider"/>
        <view class="preview-flows"><view v-for="flow in preview.flows" :key="`${flow.itemId}-${flow.toUserId}`" class="preview-flow"><text class="preview-direction">{{ flow.fromName }} → {{ flow.toName }}</text><text class="cl-field-title">{{ flow.itemTitle }}</text><text class="cl-hint">对方想要：{{ flow.matchedCategoryName }}</text></view></view>
        <view class="cl-notice"><text class="cl-field-title">请确认这是你愿意参与的方案</text><text class="cl-hint">邀请发出后，物品会为本次交换保留。大家都确认参加后，就可以约定交接。</text></view>
...
<style scoped>
.your-exchange{display:flex;flex-direction:column;gap:8px;padding:16px;margin-bottom:20px;border-radius:12px;background:var(--cl-primary-soft);font-weight:650;overflow-wrap:anywhere}.section-tabs{margin:24px 0}.match-notice .cl-hint{max-width:620px;line-height:1.8}
...
.section-tabs{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:24px 0}.section-tab{display:flex;align-items:center;gap:14px;padding:20px 24px;border:1px solid var(--cl-border);border-radius:18px;background:var(--cl-surface);color:var(--cl-muted);text-align:left}.section-tab>view{display:flex;flex-direction:column;gap:5px;font-size:18px;font-weight:700}.section-tab.selected{border:2px solid var(--cl-primary);background:var(--cl-primary-soft);color:var(--cl-primary);padding:19px 23px}.section-tab .cl-hint{font-weight:400}@media(max-width:550px){.section-tab{padding:14px;gap:8px}.section-tab.selected{padding:13px}.section-tab>view{font-size:15px}.section-tab .cl-hint{font-size:10px}}
.match-art{display:flex;flex-direction:column;align-items:center;gap:8px;flex:none}.art-bubble{padding:8px 16px;border-radius:16px 16px 4px 16px;background:var(--cl-surface);font-size:12px;color:var(--cl-primary)}.art-exchange{display:flex;align-items:center;gap:10px}.art-exchange image{height:95px;width:95px;border-radius:18px}.match-heading{background:linear-gradient(120deg,var(--cl-primary-soft),var(--cl-blue-soft))}.community-feed,.match-list{animation:page-arrive .24s var(--cl-ease)}@media(max-width:650px){.match-art{display:none}}
.match-basis{display:flex;flex-direction:column;gap:8px;align-items:flex-end;max-width:170px;text-align:right}.match-basis .cl-hint{font-size:11px;line-height:1.7}.recommendation-help{margin-top:22px;line-height:1.8}
</style>
