...
const demandStatus = demand => {
 if(demand.status!=='ACTIVE')return '已停用'
 const source=demand.offeredItems?.find(item=>item.itemId===demand.sourceItemId)
 if(demand.sourceItemId&&source?.status==='HIDDEN')return '已下架，需求暂停'
 if(demand.sourceItemId&&source?.status==='PENDING_REVIEW')return '待审核'
 if(demand.sourceItemId&&source?.status==='REJECTED')return '审核未通过'
 if(demand.sourceItemId&&source?.status==='EXCHANGED')return '已完成交换'
 if(demand.sourceItemId&&source?.status==='RESERVED')return '交换确认中'
 return demand.offeredItems?.some(item=>item.offerable)?'进行中':'暂无可交换物品'
}
const manageItem = demand => uni.navigateTo({url:`/pages/detail/detail?id=${demand.sourceItemId}&mine=1`})
...
<template>
  <LoopLayout>
    <view class="demand-heading"><view class="cl-page-heading"><text class="cl-title">我的求换清单</text><text class="cl-subtitle">发布物品时填写的求换内容会自动出现在这里，也可以另外添加想要的东西。</text></view><LoopButton v-if="authenticated" class="cl-btn cl-btn--primary" :disabled="loading || !!actionBusy" @click="openCreate">＋ 新建需求</LoopButton></view>
    ...
    <view v-if="editorOpen" class="editor-grid">
      ...
      <view class="cl-panel offer-panel"><view class="editor-head"><view><text class="cl-section-title">我有什么</text><text class="cl-hint">选择愿意用来交换的物品。</text></view><text class="cl-tag">已选 {{ selectedItemIds.length }} / 100</text></view><view v-if="selectedDetails.length" class="selected-list"><view v-for="item in selectedDetails" :key="item.itemId" class="selected-item"><text>{{ item.title || `物品 #${item.itemId}` }}</text><text v-if="item.offerable===false" class="cl-error">关联已失效</text><LoopButton class="cl-icon-btn" aria-label="移除关联物品" :disabled="editorBusy || writeUncertain || !!conflictServer" @click="removeSelected(item.itemId)">×</LoopButton></view></view><LoopSkeleton v-if="offerableLoading" :count="2"/><view v-else-if="offerableError" class="inline-state" role="alert"><text class="cl-error">{{ offerableError }}</text><LoopButton class="cl-btn" @click="loadOfferable()">重试</LoopButton></view><view v-else-if="!offerable.length" class="cl-empty compact"><text>当前没有可提供物品</text><text class="cl-hint">需求仍可保存和管理；物品审核通过后，可以在这里选择。</text><LoopButton class="cl-btn" @click="publish">发布物品</LoopButton></view><view v-else class="offerable-list"><LoopButton v-for="item in offerable" :key="item.itemId" class="offerable-item" :class="{selected:selectedItemIds.includes(item.itemId)}" :aria-pressed="selectedItemIds.includes(item.itemId)" :disabled="editorBusy || writeUncertain || !!conflictServer" @click="toggleItem(item)"><view><text class="cl-field-title">{{ item.title }}</text><text class="cl-hint">成色 {{ item.conditionLevel }} / 5 · 可交换</text></view><text>{{ selectedItemIds.includes(item.itemId) ? '已关联 ✓' : '选择' }}</text></LoopButton></view><view v-if="offerablePages>1" class="pagination"><LoopButton class="cl-btn" :disabled="offerableLoading || offerablePage<=1" @click="loadOfferable(offerablePage-1)">上一页</LoopButton><text class="cl-label">{{ offerablePage }} / {{ offerablePages }}</text><LoopButton class="cl-btn" :disabled="offerableLoading || offerablePage>=offerablePages" @click="loadOfferable(offerablePage+1)">下一页</LoopButton></view></view>
    </view>
  </LoopLayout>
</template>
