...
const categories = ref([]), items = ref([]), keyword = ref(''), selected = ref(''), total = ref(0), page = ref(1), loading = ref(false), error = ref('')
const size = 12
let sequence = 0
async function load() {
  const requestId = ++sequence; loading.value = true; error.value = ''
  try {
    const data = await http.get('/api/items', { page:page.value,size,keyword:keyword.value.trim(),...(selected.value ? {categoryId:selected.value} : {}) }, {silent:true})
    if (requestId !== sequence) return
    items.value = data.records; total.value = data.total
  } catch (e) { if(requestId === sequence) error.value = e.message }
  finally { if(requestId === sequence) loading.value = false }
}
async function init() { try { categories.value = await http.get('/api/categories',{}, {silent:true}) } catch {} await load() }
...
<template>
  <LoopLayout>
    <view class="search-row"><view class="search-field"><text class="search-symbol">⌕</text><input v-model="keyword" class="search-input" placeholder="搜一搜，让需要与闲置相遇" aria-label="搜索物品" confirm-type="search" @confirm="search"/><LoopButton v-if="keyword" class="cl-icon-btn clear-search" aria-label="清空搜索" @click="keyword='';search()">×</LoopButton><LoopButton class="cl-btn cl-btn--primary" :disabled="loading" @click="search">搜索</LoopButton></view><text class="search-note cl-desktop-only">每一件闲置，都值得下一次心动</text></view>
    ...
    <view v-if="error" class="cl-panel cl-empty"><text class="cl-empty-symbol">↺</text><text>{{ error }}</text><LoopButton class="cl-btn" @click="init">重新加载</LoopButton></view>
    <view v-else-if="loading" class="cl-empty"><text class="cl-label">正在寻找校园好物…</text></view>
    <view v-else-if="!items.length" class="cl-panel cl-empty"><text class="cl-empty-symbol">⌕</text><text>还没有找到合适的物品</text><text class="cl-hint">试试其他关键词，或发布第一件闲置。</text></view>
    <view v-else class="item-grid"><ItemCard v-for="item in items" :key="item.id" :item="item" /></view>
    <view class="pagination" v-if="total > size"><LoopButton class="cl-btn" :disabled="page === 1 || loading" @click="changePage(-1)">上一页</LoopButton><text class="cl-label">{{ page }} / {{ Math.ceil(total/size) }}</text><LoopButton class="cl-btn" :disabled="page*size >= total || loading" @click="changePage(1)">下一页</LoopButton></view>
