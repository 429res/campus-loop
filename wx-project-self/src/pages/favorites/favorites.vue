...
<template>
  <LoopLayout>
    ...
    <view v-if="!uni.getStorageSync(TOKEN_KEY)" class="cl-panel cl-empty"><text class="cl-empty-symbol">♡</text><text>登录后查看跨设备收藏</text><LoopButton class="cl-btn cl-btn--primary" @click="login">登录</LoopButton></view>
    ...
    <view v-else-if="!records.length" class="cl-panel cl-empty"><text class="cl-empty-symbol">♡</text><text>还没有收藏物品</text><text class="cl-hint">在发现页或物品详情中收藏，刷新后仍会从服务器读取。</text></view>
    <template v-else>
