...
async function read(row){if(writing.value)return;writing.value=true;const token=uni.getStorageSync(TOKEN_KEY)
 ...
 if(/^\/pages\/(exchanges|my-items|governance|history)\/[^\s]+$/.test(row.link))uni.navigateTo({url:row.link});else if(row.link==='/pages/profile/profile')uni.switchTab({url:row.link});else await load()
