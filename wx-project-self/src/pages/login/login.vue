...
onLoad(options => {
  if (['publish','demands','matches','favorites'].includes(options.redirect)) redirect = options.redirect
...
async function login() {
  ...
  try {
    ...
    if (['demands','favorites'].includes(redirect)) uni.redirectTo({url:`/pages/${redirect}/${redirect}`})
    else uni.switchTab({url:`/pages/${redirect}/${redirect}`})
  } catch(e) { error.value = e.message } finally { busy.value = false }
}
</script>
<template>
  <LoopLayout><view class="login-layout"><view class="login-copy"><text class="login-kicker">WELCOME BACK</text><text class="login-title">好物的下一站，<br/>从这里开始。</text><text class="cl-subtitle">把闲置交给需要的人，<br/>也找到你的校园小确幸。</text><image src="/static/demo/book.svg" mode="aspectFit" class="login-art"/></view><view class="cl-panel login-panel"><text class="cl-title">登录 Campus Loop</text><text class="cl-subtitle login-intro">使用在本地初始化时创建的开发账号</text><text v-if="notice" class="cl-notice login-notice" role="status">{{ notice }}</text><form class="cl-form" @submit="login"><view class="cl-field"><text class="cl-field-title">用户名</text><input v-model="username" class="cl-input" aria-label="用户名" placeholder="输入用户名" :aria-invalid="!!error" :disabled="busy" maxlength="60" /></view><view class="cl-field"><text class="cl-field-title">密码</text><input v-model="password" class="cl-input" aria-label="密码" placeholder="输入密码" password :disabled="busy" maxlength="128" confirm-type="done" @confirm="login" /></view><text v-if="error" class="cl-error" role="alert">{{ error }}</text><LoopButton class="cl-btn cl-btn--primary cl-btn--wide" form-type="submit" :disabled="busy" :loading="busy">{{ busy ? '登录中…' : '登录，开始循环' }}</LoopButton><text class="cl-hint">账号由团队成员在各自开发环境创建。注册、找回密码及微信授权将在后续阶段实现。</text></form></view></view></LoopLayout>
</template>
<style scoped>
