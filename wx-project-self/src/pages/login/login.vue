...
<template>
  <LoopLayout><view class="login-layout"><view class="login-copy"><text class="login-kicker">WELCOME BACK</text><text class="login-title">好物的下一站，<br/>从这里开始。</text><text class="cl-subtitle">把闲置交给需要的人，<br/>也找到你的校园小确幸。</text><image src="/static/demo/book.svg" mode="aspectFit" class="login-art"/></view><view class="cl-panel login-panel"><text class="cl-title">登录 Campus Loop</text><text v-if="notice" class="cl-notice login-notice" role="status">{{ notice }}</text><form class="cl-form" @submit="login"><view class="cl-field"><text class="cl-field-title">用户名</text><LoopInput v-model="username" class="cl-input" aria-label="用户名" placeholder="输入用户名" :aria-invalid="!!error" :disabled="busy" maxlength="64" /></view><view class="cl-field"><text class="cl-field-title">密码</text><LoopInput v-model="password" class="cl-input" aria-label="密码" placeholder="输入密码" password :disabled="busy" maxlength="128" confirm-type="done" @confirm="login" /></view><text v-if="error" class="cl-error" role="alert">{{ error }}</text><LoopButton class="cl-btn cl-btn--primary cl-btn--wide" form-type="submit" :disabled="busy" :loading="busy">{{ busy ? '登录中…' : '登录' }}</LoopButton><LoopButton class="cl-btn cl-btn--wide" :disabled="busy" @click="openRegister">注册账号</LoopButton></form></view></view></LoopLayout>
</template>
<style scoped>
