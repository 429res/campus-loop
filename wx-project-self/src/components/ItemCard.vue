...
<template>
  <view class="cl-card item-card">
    ...
    <LoopButton v-if="showFavorite" class="favorite-button" :class="{active:favorited}" :disabled="favoriteBusy" :aria-label="favorited ? `取消收藏 ${item.title}` : `收藏 ${item.title}`" @click.stop="emit('favorite',item)">{{ favoriteBusy ? '…' : favorited ? '♥' : '♡' }}</LoopButton>
  </view>
</template>
<style scoped>
