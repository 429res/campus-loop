...
public interface HistoryMapper {
    ...
    String CONFIRMATION_ACCESS="EXISTS (SELECT 1 FROM cl_history_confirmation_member m WHERE m.history_id=h.id AND m.user_id=#{actor})";
    String VISIBLE="(#{all}=TRUE OR h.source_user_id=#{actor} OR "+CONFIRMATION_ACCESS+" OR "+PAIR+")";
    ...
    @Select("SELECT "+COLUMNS+FROM+"WHERE h.id=#{id} FOR UPDATE")
    HistoryEvent findCurrent(long id);
    @Select("SELECT COUNT(*) FROM cl_item_history h WHERE h.id=#{id} AND (h.source_user_id=#{actor} OR "+CONFIRMATION_ACCESS+" OR "+PAIR+")")
    ...
    @Select("SELECT COUNT(*) FROM cl_history_evidence v JOIN cl_item_history h ON h.id=v.history_id WHERE v.upload_id=#{upload} AND (#{admin}=TRUE OR h.source_user_id=#{actor} OR "+CONFIRMATION_ACCESS+" OR "+PAIR+")")
