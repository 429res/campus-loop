...
public class CommunityService {
 ...
 public record Post(long id,long authorId,String authorName,String avatarUrl,String body,Long itemId,String itemTitle,String itemImage,String imageUrl,String status,int version,long likes,long replies,boolean liked,long reports,LocalDateTime createdAt,List<Reply> previewReplies,String itemStatus){}
 ...
 public PageResult<Post> page(Long viewer,int page,int size,boolean mine,boolean admin,boolean reported,String keyword){
  ...
  return new PageResult<>(posts.stream().map(p->new Post(p.id(),p.authorId(),p.authorName(),p.avatarUrl(),p.body(),p.itemId(),p.itemTitle(),p.itemImage(),p.imageUrl(),p.status(),p.version(),p.likes(),p.replies(),p.liked(),p.reports(),p.createdAt(),previews.stream().filter(r->r.postId()==p.id()).toList(),p.itemStatus())).toList(),count,page,size);
 }
