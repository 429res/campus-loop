...
public class CommunityService {
 ...
 public record ReplyRequest(@NotBlank @Size(max=1000) String body,@NotBlank @Pattern(regexp="[a-zA-Z0-9_-]{8,64}") String requestKey){}
 ...
 public record Post(long id,long authorId,String authorName,String avatarUrl,String body,Long itemId,String itemTitle,String itemImage,String imageUrl,String status,int version,long likes,long replies,boolean liked,long reports,LocalDateTime createdAt){}
 public record Reply(long id,long postId,long authorId,String authorName,String avatarUrl,String body,LocalDateTime createdAt){}
 ...
 public PageResult<Post> page(Long viewer,int page,int size,boolean mine,boolean admin,boolean reported,String keyword){
  ...
  return new PageResult<>(db.query(POST_SELECT+where+" ORDER BY p.id DESC LIMIT ? OFFSET ?",(rs,n)->post(rs,admin),read.toArray()),count,page,size);
 }
 ...
 public PageResult<Reply> replies(long id,Long actor,int page,int size,boolean admin){
  ...
  String from=" FROM cl_community_reply r JOIN cl_user u ON u.id=r.author_id WHERE r.post_id=? AND r.status='PUBLISHED' AND u.status='ACTIVE'";
  long total=db.queryForObject("SELECT COUNT(*)"+from,Long.class,id);
  var rows=db.query("SELECT r.*,u.display_name,u.avatar_url"+from+" ORDER BY r.id ASC LIMIT ? OFFSET ?",(rs,n)->new Reply(rs.getLong("id"),id,rs.getLong("author_id"),rs.getString("display_name"),rs.getString("avatar_url"),rs.getString("body"),rs.getObject("created_at",LocalDateTime.class)),id,size,(long)(page-1)*size);
  return new PageResult<>(rows,total,page,size);
 }
 public long reply(long actor,long id,ReplyRequest body){
  lockParticipants(actor,id);var post=lockPost(id);published(post);
  var previous=db.queryForList("SELECT id,post_id,body FROM cl_community_reply WHERE author_id=? AND request_key=?",actor,body.requestKey());
  if(!previous.isEmpty()){var p=previous.get(0);if(((Number)p.get("post_id")).longValue()!=id||!p.get("body").equals(body.body().trim()))throw new ApiException(409,"回复内容已变化，请重新发送");return ((Number)p.get("id")).longValue();}
  ...
  db.update("INSERT INTO cl_community_reply(post_id,author_id,body,request_key,created_at) VALUES(?,?,?,?,?)",id,actor,body.body().trim(),body.requestKey(),now());
 ...
 private User lockParticipants(long actor,long postId){
  ...
  User result=null;for(long id:new TreeSet<>(List.of(actor,authors.get(0)))){var user=users.selectByIdForUpdate(id);if(id==actor)result=user;}
