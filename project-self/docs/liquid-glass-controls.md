...
- 账号与社区升级：千问审核建议面板沿用 Element Plus 按钮/告警/标签；物品多图使用完整适配与图片预览；曝光排序和删除账号复用统一确认弹层。

- 自动审核队列：AutoReviewQueue 复用 panel、标签、按钮与告警；每 15 秒更新状态，销毁时停止轮询。支持失败提示、刷新禁用、人工接管和响应式换行；详情面板展示 AI 来源与审核理由。
